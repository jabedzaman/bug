...
import SpotidlError from "./lib/Error";
import getYtlink from "./lib/getYtlink";
import metadata from "./lib/metadata";

export default class Fetcher extends SpotifyApi {
  ...
  downloadTrack = async <T extends undefined | string>(
    ...
    if (!link)
      throw new SpotidlError(
        `Couldn't get a download URL for the track: ${info.name}`
      );
  ...
  private downloadBatch = async (
    ...
    await this.verifyCredentials();
    const playlist = await this[type === "album" ? "getAlbum" : "getPlaylist"](
      url
    );
    return Promise.all(
  ...
  downloadPlaylist = async (url: string): Promise<(string | Buffer)[]> =>
    await this.downloadBatch(url, "playlist");
  ...
  downloadAlbum = async (url: string): Promise<(string | Buffer)[]> =>
    await this.downloadBatch(url, "album");
  ...
  getTracksFromPlaylist = async (
    ...
    const tracks = await Promise.all(
      playlist.tracks.map((track) => this.getTrack(track))
    );
    return {
      name: playlist.name,
  ...
  /**
   * Gets the info of user from ID
   * @param id ID of the user
   */
  getSpotifyUser = async (id: string): Promise<UserObjectPublic> =>
